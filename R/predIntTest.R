...
predIntTest <- function(reconDat, envelope=0.9){
  ...
  for (setAsideLoop in 1:possibleIts1){
    ...
    for (i in 1:(possibleIts+1)){
      ...
      calibCI95 <- as.numeric(quantile(upperDif, probs=envelopeUpper))
      calibCI05 <- as.numeric(quantile(lowerDif, probs=envelopeLower))
      ...
      #verifPI95 <- as.numeric(quantile(vUpperDif, probs = envelopeUpper))
      #verifPI05 <- as.numeric(quantile(vLowerDif, probs = envelopeLower))
      #verifPI05 <- interpIndex(vLowerDif, envelopeLower)
      errorSet <- abs(reconDat$Target[verif1] - reconDat$reconstruction[verif1])
      trueRMSE <- as.numeric(quantile(errorSet, probs=envelope))
