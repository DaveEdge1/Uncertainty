...
trailDir <- "FourChronTrialsAug222023/"
devtools::load_all(paste0(uncertaintiesDir,"R"))


#load Tree Nob detrended ring width data
climDat1 <- read.csv(paste0(uncertaintiesDir, "SimpleBootTrials/LangaraSST.csv"))
climDat1 <- climDat1[climDat1$Year < 2002,]
TNchron <- read.rwl(paste0(uncertaintiesDir,"SimpleBootTrials/TreeNobRCStrunc1.csv"))
...
climDat4 <- read.csv(paste0(uncertaintiesDir,"SimpleBootTrials/canaTempTarget.csv"))
head(climDat4)
climDat4 <- climDat4[,-1]
names(climDat4) <- c("Year", "Target")
chron113 <- read.rwl(paste0(uncertaintiesDir,"SimpleBootTrials/cana113.rwl"))
...
saveRDS(TreeNobSimTest, file = paste0(uncertaintiesDir, trailDir,"TreeNobSimTest.RData"))
rm(TreeNobSimTest)
...
TreeNobTest <- fullTestingIntervalsNew(
  ...
  skipChron=FALSE,
  halfSampleDepth=0)
saveRDS(TreeNobTest, file = paste0(uncertaintiesDir, trailDir, "TreeNobTest.RData"))
rm(TreeNobTest)
...
ca646SimTest <- fullTestingIntervalsNew(
  ...
  skipChron=TRUE,
  halfSampleDepth=0.25)
saveRDS(ca646SimTest, file = paste0(uncertaintiesDir, trailDir, "ca646SimTest.RData"))
rm(ca646SimTest)
...
ca646Test <- fullTestingIntervalsNew(
  ...
  skipChron=FALSE,
  halfSampleDepth=0)
saveRDS(ca646Test, file = paste0(uncertaintiesDir, trailDir, "ca646Test.RData"))
rm(ca646Test)
...
cana113SimTest <- fullTestingIntervalsNew(
    ...
    skipChron=TRUE,
    halfSampleDepth=0.25)
saveRDS(cana113SimTest, file = paste0(uncertaintiesDir, trailDir, "cana113SimTest.RData"))
rm(cana113SimTest)
...
cana113test <- fullTestingIntervalsNew(
  ...
  skipChron=FALSE,
  halfSampleDepth=0)
saveRDS(cana113test, file = paste0(uncertaintiesDir, trailDir, "cana113test.RData"))
rm(cana113test)
