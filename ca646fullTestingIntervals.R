...
#load data
climDat1 <- read.csv("C:/Users/dce72/Documents/GradSchool/Uncertainty/SimpleBootTrials/ca646PrecipTarget.csv")
climDat1 <- climDat1[,-1]
names(climDat1) <- c("Year", "Target")
# climDat1 <- climDat1[climDat1$Year < 2002,]
TNchron <- read.rwl("C:\\Users\\dce72\\Documents\\GradSchool\\Uncertainty\\SimpleBootTrials\\ca646rwi.csv")
# TNchron <- TNchron[as.numeric(rownames(TNchron))>1725,]
# TNchron <- TNchron[,apply(TNchron, 2, function(x) sum(!is.na(x)))>0]
